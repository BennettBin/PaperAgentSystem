...
from backend.agent_runtime.planner import ExecutionPlan
...
class ToolInvoker(ActionInvoker, Protocol):
        ...
        *,
        ...
        for step_id in plan.topological_order():
            ...
            if cached is not None:
                ...
                continue
            if step.tool_name:
                if not self._tools.has(step.tool_name):
                    raise ProjectError(
                ...
                try:
                    output = await asyncio.wait_for(
                        self._tools.invoke(step.tool_name, arguments),
