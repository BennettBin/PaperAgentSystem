...
description: 按用户目标总结论文内容，保留关键数字与证据覆盖。 Use when: 总结论文、章节或实验. Do not use when: 核验单个主张；综合多篇论文.
---
...
- Description: 按用户目标总结论文内容，保留关键数字与证据覆盖。
- Trigger: 总结论文、章节或实验。
- Do not trigger: 核验单个主张；综合多篇论文。
- Version: 1.0.0
- Input format: `object`
...
| Tool | 用途 | 何时调用 |
|---|---|---|
| `search_document` | 检索摘要目标对应证据 | 生成摘要前 |
