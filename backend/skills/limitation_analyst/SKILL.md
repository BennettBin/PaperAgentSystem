---
name: limitation_analyst
description: 区分作者明确陈述的局限与审阅者基于证据推断的局限。 Use when: 分析局限；查找未来工作. Do not use when: 通用方法评审；把推断冒充原话.
---
...
- Description: 区分作者明确陈述的局限与审阅者基于证据推断的局限。
- Trigger: 分析局限；查找未来工作。
- Do not trigger: 通用方法评审；把推断冒充原话。
- Version: 1.0.0
- Input format: `object`
...
| Tool | 用途 | 何时调用 |
|---|---|---|
| `search_document` | 检索局限、威胁和未来工作证据 | 分析前 |
