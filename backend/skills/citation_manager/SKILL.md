...
description: 核验和整理系统已分配的引用标识，确保引用与来源一致。 Use when: 核验引用是否有效；整理参考文献或引用映射. Do not use when: 生成不存在的引用；不涉及引用的普通问答.
---
...
- Description: 核验和整理系统已分配的引用标识，确保引用与来源一致。
- Trigger: 核验引用是否有效；整理参考文献或引用映射。
- Do not trigger: 生成不存在的引用；不涉及引用的普通问答。
- Version: 1.0.0
- Input format: `object`
...
| Tool | 用途 | 何时调用 |
|---|---|---|
| `search_document` | 检索引用上下文与来源证据 | 引用目标或来源需要定位时 |
