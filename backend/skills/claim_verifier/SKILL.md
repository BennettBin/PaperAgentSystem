---
name: claim_verifier
description: 依据给定证据判定主张为支持、矛盾或证据不足。 Use when: 核验明确主张；检查结论证据. Do not use when: 抽取主张；无证据的一般问答.
---
...
- Description: 依据给定证据判定主张为支持、矛盾或证据不足。
- Trigger: 核验明确主张；检查结论证据。
- Do not trigger: 抽取主张；无证据的一般问答。
- Version: 1.0.0
- Input format: `object`
...
| Tool | 用途 | 何时调用 |
|---|---|---|
| `search_document` | 检索支持或矛盾证据 | 判定主张前 |
...
- `references/evidence_labels.md`：领域判定或执行细则；相关任务执行前按需读取。
