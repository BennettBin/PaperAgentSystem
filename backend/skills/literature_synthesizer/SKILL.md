...
description: 从多篇论文的证据矩阵生成可追溯的综合性综述。 Use when: 综合多篇文献；撰写证据化综述. Do not use when: 只比较字段；总结单篇论文.
---
...
- Description: 从多篇论文的证据矩阵生成可追溯的综合性综述。
- Trigger: 综合多篇文献；撰写证据化综述。
- Do not trigger: 只比较字段；总结单篇论文。
- Version: 1.0.0
- Input format: `object`
...
| Tool | 用途 | 何时调用 |
|---|---|---|
| `search_document` | 跨论文检索主题证据 | 建立证据矩阵前 |
| `build_literature_review` | 从主题和证据矩阵生成综述 | 主题归类完成后 |
| `save_artifact` | 保存用户要求的长期成果 | 需要下载或长期管理时 |
