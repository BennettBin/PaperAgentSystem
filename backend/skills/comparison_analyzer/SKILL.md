...
description: 按统一维度比较至少两篇论文并保留逐项证据。 Use when: 比较多篇论文；生成结构化对比表. Do not use when: 只阅读一篇；跨文献叙事综述.
---
...
- Description: 按统一维度比较至少两篇论文并保留逐项证据。
- Trigger: 比较多篇论文；生成结构化对比表。
- Do not trigger: 只阅读一篇；跨文献叙事综述。
- Version: 1.0.0
- Input format: `object`
...
| Tool | 用途 | 何时调用 |
|---|---|---|
| `search_document` | 逐篇检索统一维度证据 | 构建矩阵前 |
| `build_comparison_table` | 归一化字段并生成证据矩阵 | Paper Card 齐备后 |
