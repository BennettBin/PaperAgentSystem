...
1. 创建根目录：
   - `apps/`
   - `backend/`（API、Worker、Agent、Memory、RAG、Skill、Tool 与基础设施适配器）
   - `frontend/`
...
早期 Mock API、开发演示组件和数据管理占位页已在产品入口接入真实 API 后移除；
新功能必须接入真实产品服务或明确 fail-closed。
...
将首页从 Mock 状态接入真实 Conversation、File、Queue、PDF Parser、RAG 和 LLM Port。

### 功能

- 新对话立即创建后端会话记录。
- 最近对话支持关键词过滤和历史消息回读。
- 文件库展示当前 Workspace 的上传文件。
- Composer 内展示上传入口和已上传文件，移除无实现的快捷按钮。
- 上传 PDF 后投递解析任务，Worker 执行解析、分块和索引。
- 发送消息后投递主任务，Worker 检索证据并调用 OpenAI-compatible 模型。
- 模型不可用时任务明确失败，不生成伪回答。

### 验收标准

- 前端交互测试覆盖新对话、搜索、历史切换和上传文件展示。
- 后端回归测试证明 PDF 文本进入检索结果和模型 Prompt。
- API、Worker、前端类型检查和构建通过。
- Docker Compose 启动真实 API/Worker 集成链路。

---

## K02：模型设置与本地 Base/SFT/RL 版本管理

### 目标

提供统一模型设置界面，并使用真实 Ollama 服务管理 1.7B 与 4B 逻辑 Profile。

### 功能

- 左侧底部提供“模型配置”入口，设置页保持主界面视觉风格。
- 分别选择小模型（1.7B）与大模型（4B）的 Base、SFT 或 RL 版本。
- 默认选择 Base；SFT/RL 只有存在唯一 Manifest 且已晋级时才可选。
- 展示模型不可重复 ID、服务名、后训练版本、安装和探活状态。
- 支持检查其他 Base 模型；本地缺失时提示并通过 Ollama API 下载。
- Worker 按数据库中的当前选择调用模型，不硬编码物理路径。

### 验收标准

- 模型配置 API、下载/探活和选择持久化具有自动测试。
- 1.7B 与 4B Base 均完成真实 OpenAI-compatible 推理验收。
- 前端模型设置交互、TypeScript、构建和页面验收通过。
- 模型选择、下载结果和版本信息进入文档与过程日志。
...
| K04.2 | completed | 2026-06-22 | Section Reference Parser、Resolver、别名/模糊匹配、歧义处理与固定 100-query 评测 |
| K04.3 | completed | 2026-07-21 | 确定性 Resolver 接入、SQL 精确 scope、父子章节展开、QA rerank/相邻块、顺序覆盖式章节总结与产品澄清 metadata；39 项相关回归通过 |
| K04.4 | completed | 2026-07-21 | 显式索引版本与自动重建、结构化隐私 Trace、非法 Evidence ID 阻断、真实三页多章节 PDF E2E；完整 303 项 pytest 通过 |
| L00 | completed | 2026-07-21 | 冻结 Vanilla RAG、固定 Workflow、受限 ReAct、全 4B 四套 Baseline；记录环境/工作区，建立真实性分级与 Fake 效果门禁，完成现状审计和框架边界 ADR；下一工作包 L01 |
| L01 | completed | 2026-07-21 | 冻结 L1～L6 EvaluationCase/Trajectory/Reference/Evidence/Judge 契约；按 paper、conversation、来源簇、文本指纹和 Embedding 聚类阻断跨 split 泄漏；未授权及未同意私有数据拒绝加载；下一工作包 L02 |
