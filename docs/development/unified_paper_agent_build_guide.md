...
│   ├── agent_runtime/          # Agent 编排、规划、执行与核验
│   ├── memory/                 # 短期记忆与长期记忆
│   ├── skills/                 # Skill 定义、注册与实现
│   ├── tool_runtime/           # Skill 可调用的工具实现
│   ├── document_processing/    # PDF/OCR/版面解析
│   ├── rag/                    # 索引、检索与引用
│   ├── models/                 # 模型路由与运行时
│   └── infrastructure/         # PostgreSQL、Redis、MinIO 等适配器
├── frontend/                   # Next.js/React/TypeScript
├── infrastructure/
│   ├── docker/                 # Compose、Dockerfile 与 OTEL
│   └── database/               # Alembic 入口配置
├── runtime/                    # 日志、诊断与临时运行产物
├── evaluation/
├── training/
├── tests/
├── scripts/
