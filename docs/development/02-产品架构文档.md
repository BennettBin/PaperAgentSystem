...
通过受控图片 API 返回并在消息下方展示。会话/文件删除同步释放视觉对象引用。

产品回答链路接入相关多轮上下文选择器。它使用指代/追问标记和主题词重合判断是否需要历史，
最多回读 8 条、3600 字符的原始 user/assistant 消息，并把来源 message IDs 写入 RAG metadata。
相关历史同时提供给 Self-RAG 决策与最终回答 Prompt；独立新问题不注入历史，全部会话历史不会
无界进入模型上下文。该在线选择器补齐了已有 Memory 基础设施与产品入口之间的连接。
...
多轮选择器额外识别“列举、举例、逐一、逐个、分别说明、具体有哪些”等省略式追问。命中后，
