...
机制参考：`backend/subagents/review_loop.py`
在线执行：`backend/subagents/role_runner.py` 与 `backend/subagents/runtime_adapter.py`
...
默认 Worker 在 `backend/apps/worker/runtime.py` 中创建 `MultiAgentRuntimeAdapter`，但只有
`MULTI_AGENT_ENABLED=true` 和 `ALLOW_EXPERIMENTAL_NO_GO=true` 同时成立、论文数不少于两篇且
用户明确要求比较/综述/综合分析时才调用。Coordinator 为每个 `file_id` 创建一个并行 Reader；
Reader 复用 `PaperReaderAgent`，通过 Manifest 白名单内的 `search_document` Tool 只访问自己的
论文。各 Reader 将 Paper Card 写入任务级 Blackboard，Evidence 再统一分配公开 `E#` 并保留
file/page/section/bbox，Critic 审查 Matrix，Writer 只能基于 Matrix 写作，Verifier 做结构化与
确定性双重检查。

严重问题最多触发一次 `writer:revision` 和一次 `verifier:revision`。最终结果回到
`PaperAgentProcessor` 后还要经过产品边界 Verifier，只有全部引用能映射到持久化 Evidence 才
保存 Message；视觉材料和 `memory_summary` 继续走原链。Critic 失败可降级，Reader 失败必须
披露缺失论文，Evidence/Writer/Verifier 或第二次核验失败则不保存答案。每个角色在任务监控、
SQL Trace、JSONL 日志和 Blackboard 中都能审计，但不会记录隐藏推理或完整 Prompt。
...
因此系统保留可由双开关显式运行的真实多智能体实现，但默认采用 Single Agent/Safe RAG。
这说明项目具备“算法实验—成本评估—产品晋级”的闭环。
...
| `runtime.py` | 默认 Worker 的依赖装配，以及解析、主 Agent、Memory 摘要处理器注册 |
| `fake_queue.py` | 测试用队列 |
| `main.py` | Worker 入口 |
