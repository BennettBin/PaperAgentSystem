...
from backend.infrastructure.postgres.blackboard import SqlAlchemyBlackboardRepository
...
@pytest.mark.parametrize("factory", [_fake, _sql])
@pytest.mark.asyncio
...
async def test_workspace_isolation_and_source_deletion_invalidate_retrieval(factory) -> None:
    ...
    try:
        ...
        assert len(await repository.list_active("ws-1", "task-1")) == 1
